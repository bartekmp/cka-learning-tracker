...
async function autoSave() {
  if (!fileHandle) return;
  try {
    if (!(await canWrite(fileHandle))) { setBarState("perm", null, taskCbs()); return; }
    await writeHandle(fileHandle, done, FILE_MARKER);
    asFlash("✓ Saved");
  } catch { asFlash("Save failed", "#dc2626"); }
}

function manualExport() {
  const blob = new Blob(
    [JSON.stringify({ _type: FILE_MARKER, saved: new Date().toISOString(), data: done }, null, 2)],
...
function manualImport(evt) {
  ...
  reader.onload = e => {
    try {
      ...
      if (p._type !== FILE_MARKER) { asFlash("Wrong file", "#dc2626"); return; }
      done = p.data || {};
      refreshAllButtons();
      updateOverall();
      asFlash("✓ Imported!");
    } catch { asFlash("Could not read file", "#dc2626"); }
    evt.target.value = "";
  };
...
function refreshAllButtons() {
  document.querySelectorAll(".done-btn").forEach(btn => {
    const tid = btn.dataset.taskid; if (!tid) return;
...
async function initStorage() {
  if (!HAS_FSA) { setBarState("fallback", null, taskCbs()); return; }
  const h = await idbGet(HANDLE_KEY);
  if (!h) { setBarState("none", null, taskCbs()); return; }
  fileHandle = h;
  if (!(await canWrite(h))) { setBarState("perm", null, taskCbs()); return; }
  try {
...
function updateOverall() {
  ...
  document.getElementById("overall-label").textContent = `${completedTasks} / ${totalTasks} tasks completed`;
}
...
function copyCode(el, text) {
  ...
  if (hint) { hint.textContent = "✓ copied"; setTimeout(() => hint.textContent = "click to copy", 1600); }
}
...
function buildPanels() {
  ...
  SECTIONS.forEach((s, si) => {
    ...
    s.tasks.forEach((task, ti) => {
      ...
      const diffClass = task.difficulty === "hard" ? "hard" : task.difficulty === "medium" ? "medium" : "easy";
      ...
      task.steps.forEach(step => {
        ...
        step.code.forEach(c => {
          ...
          codeEl.addEventListener("click", () => {
            const raw = codeEl.textContent.replace("click to copy", "").replace("✓ copied", "").trim();
            copyCode(codeEl, raw);
          });
      ...
      toggle.addEventListener("click", () => {
        ...
        toggle.childNodes[1].textContent = " " + (open ? "Hide solution" : "Show step-by-step solution");
      });

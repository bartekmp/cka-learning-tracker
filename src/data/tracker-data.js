		...
		topics: [
			...
			},
			{
				...
				tasks: [
					...
					'Create LoadBalancer: kubectl expose deployment <n> --type=LoadBalancer --port=80',
					'LoadBalancer EXTERNAL-IP stays <pending> without a cloud provider or MetalLB — that is expected locally',
					'Test connectivity: kubectl run tmp --image=busybox:1.28 --rm -it -- wget -O- <svc>:80',
					'Understand the three types: ClusterIP (internal), NodePort (node IP + 30000–32767), LoadBalancer (external IP via cloud/LB)',
					'Check service endpoints: kubectl get endpoints <svc> — empty means selector mismatch',
				],
				cmds: [
					'kubectl expose pod <pod> --port=80 --type=ClusterIP',
					'kubectl expose deployment <n> --port=80 --type=NodePort',
				...
				labs: [
					...
					'Expose deployment as all three types in sequence, test connectivity for ClusterIP and NodePort',
				],
			},
		],
	},
	...
	},
	{
		...
		topics: [
			...
			},
			{
				...
				tasks: [
					'Pre-flight (all nodes): disable swap — swapoff -a, remove swap from /etc/fstab',
					'Load kernel modules: overlay and br_netfilter via /etc/modules-load.d/k8s.conf',
					'Set sysctl params: net.ipv4.ip_forward=1, bridge-nf-call-iptables=1',
					'Install containerd, configure SystemdCgroup=true in /etc/containerd/config.toml',
					'Install kubeadm, kubelet, kubectl at the target version and hold them',
					'Bootstrap control plane: kubeadm init --pod-network-cidr=192.168.0.0/16',
					'Configure kubeconfig: copy /etc/kubernetes/admin.conf to ~/.kube/config',
					'Install a CNI (e.g. Calico) so pods can communicate',
					'Join workers: kubeadm token create --print-join-command → run on each worker',
				...
				cmds: [
					...
					'kubeadm join <cp-ip>:6443 --token <t> --discovery-token-ca-cert-hash <h>',
				],
				labs: [
					"Killercoda: 'Kubernetes Install' scenario (two fresh Ubuntu nodes)",
					'KodeKloud: kubeadm Installation Lab',
				],
			},
			{
				id: 's5t8',
				...
				tasks: [
					'Understand stacked etcd: etcd runs on each control plane node (simpler, default)',
					'Understand external etcd: etcd on separate dedicated nodes (more resilient)',
					'HA requires a load balancer (HAProxy, cloud LB) in front of all API servers',
					'All nodes use the same --control-plane-endpoint (the LB hostname/IP)',
					'Minimum 3 control plane nodes for etcd quorum — odd number avoids split-brain',
					'Bootstrap HA: kubeadm init --control-plane-endpoint=<lb>:6443 --upload-certs',
					'Join second control plane: kubeadm join --control-plane --certificate-key <key>',
					'Check API server endpoints: kubectl get endpoints kubernetes',
				],
				cmds: [
					'kubeadm init --control-plane-endpoint=<lb>:6443 --upload-certs',
					'kubeadm join <lb>:6443 --token <t> --discovery-token-ca-cert-hash <h> --control-plane --certificate-key <k>',
					'kubectl get endpoints kubernetes',
					'kubectl get pods -n kube-system | grep etcd',
				],
				labs: [
					'KodeKloud: HA Kubernetes Cluster Lab',
					'Conceptual: draw stacked vs external etcd topology from memory',
				],
			},
			{
				id: 's5t9',
				...
				tasks: [
					'CNI (Container Network Interface): plugins that assign pod IPs — Calico, Flannel, Cilium',
					'CSI (Container Storage Interface): plugins that provision volumes — AWS EBS, NFS, local-path',
					'CRI (Container Runtime Interface): abstraction over runtimes — containerd, CRI-O',
					"Find which runtime a node uses: kubectl get node <n> -o jsonpath='{.status.nodeInfo.containerRuntimeVersion}'",
					'Use crictl to inspect containers directly on a node (bypasses the Kubernetes API)',
					'List CSI drivers: kubectl get csidrivers',
					'List CNI config files: ls /etc/cni/net.d/',
					'crictl is essential when kubectl is unresponsive or a pod never starts',
				],
				cmds: [
					'crictl ps',
					'crictl ps -a',
				...
				labs: [
					'Killercoda Kubernetes playground: SSH to a node and use crictl to list and inspect containers',
...
const CHEATSHEET = [
	...
	},
	{
		...
		items: [
			{
				cmd: 'kubectl expose deployment <n> --port=80 --type=ClusterIP',
				desc: 'ClusterIP (cluster-internal only)',
			},
			{
				cmd: 'kubectl expose deployment <n> --port=80 --type=NodePort',
				desc: 'NodePort (node IP + 30000–32767)',
			},
			{
				cmd: 'kubectl expose deployment <n> --port=80 --type=LoadBalancer',
				desc: 'LoadBalancer (external IP via cloud/LB)',
			},
			{ cmd: 'kubectl get svc -o wide', desc: 'List services with selectors' },
			{
				cmd: 'kubectl get endpoints <svc>',
				desc: 'Check endpoint IPs — empty = selector mismatch',
			},
			{
				cmd: 'kubectl patch svc <n> -p \'{"spec":{"type":"NodePort"}}\'',
				desc: 'Change service type in-place',
			},
		],
	},
	{
	...
	},
	{
		...
		items: [
			...
			{ cmd: 'crictl ps', desc: 'Running containers on a node' },
			{ cmd: 'crictl ps -a', desc: 'All containers including exited' },
			{ cmd: 'crictl logs <container-id>', desc: 'Container logs via CRI' },
			{ cmd: 'crictl inspect <container-id>', desc: 'Full container spec and state' },
			{ cmd: 'crictl images', desc: 'Images cached on a node' },
			{ cmd: 'crictl pods', desc: 'CRI-level pod sandboxes' },
		],
	},
	{
		cat: 'JSONPath & output',
